...
class LoadAnnotations3D(LoadAnnotations):
    ...
    def _load_bboxes(self, results: dict) -> None:
        ...
        """
        # print("\n========================trainsforms loading results['ann_info'] keys {}=========================".format(results['ann_info'].keys()))
        # nusences dict keys:dict_keys(['sample_idx', 'token', 'timestamp', 'ego2global', 
        # 'images', 'lidar_points', 'instances', 'cam_instances', 'num_pts_feats', 'lidar_path', 'ann_info'])
        # 强制获得gt_bboxes
        if 'gt_bboxes' not in results['ann_info'].keys():
            results['gt_bboxes'] = None
        else:
    ...
    def _load_labels(self, results: dict) -> None:
        ...
        """
        if 'gt_bboxes_labels' not in results['ann_info'].keys():
            results['gt_bboxes_labels'] = None
        else:        
