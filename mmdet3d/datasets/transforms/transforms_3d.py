        ...
        if self.flip_box3d:
            if 'gt_bboxes_3d' in input_dict:
                if 'points' in input_dict:
                    ...
                    # vision-only detection(no points only image)
...
@TRANSFORMS.register_module()
class ObjectSampleV2(BaseTransform):
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        if self.sample_2d:
            img = input_dict['img']
            img_filename = input_dict.get('img_filename', None)       # for DEBUG only
...
# GlobalRotScaleTrans同步旋转点云和gt box 3d, 是否可以直接读取这里的gt box 3d
@TRANSFORMS.register_module()
class GlobalRotScaleTrans(BaseTransform):
    ...
    def _trans_bbox_points(self, input_dict: dict) -> None:
        ...
        # print("trans_factor : {}".format(trans_factor))
        input_dict['points'].translate(trans_factor)
        input_dict['pcd_trans'] = trans_factor
        # 需要input dict里含有gt bboxes 3d
        if 'gt_bboxes_3d' in input_dict:
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        # print("==================== GlobalRotScaleTrans forward =========================")
        # #   input_dict.keys : dict_keys(['sample_idx', 'images', 'lidar_points', 'instances', 
        # # 'cam_instances', 'plane', 'num_pts_feats', 'lidar_path', 'img_path', 'lidar2cam', 'cam2img', 'lidar2img', 
        # # 'eval_ann_info', 'box_type_3d', 'box_mode_3d', 'points', 'img', 'img_shape', 'ori_shape', 'scale', 'flip', 
        # # 'pcd_scale_factor', 'flip_direction', 'pcd_horizontal_flip', 'pcd_vertical_flip', 'scale_factor', 'keep_ratio'])

        # instances = input_dict["instances"]
        # print("====================  input_dict.keys : {} =========================".format(input_dict.keys()))
        # #   instances[0].keys : dict_keys(['bbox', 'bbox_label', 'bbox_3d', 'bbox_label_3d', 'depth', 
        # # 'center_2d', 'num_lidar_pts', 'difficulty', 'truncated', 'occluded', 'alpha', 'score', 'index', 'group_id']) 
        # print("====================  instances[0].keys : {} =========================".format(instances[0].keys()))
        
        # bbox_3d0 = instances[0]["bbox_3d"]

        # print("====================  bbox_3d0.type : {} =========================".format(type(bbox_3d0)))
        # print(" bbox 3d : {}".format(bbox_3d0))
...
# @TRANSFORMS.register_module()
# class GlobalRotScaleTrans(BaseTransform):
#     """对3D场景应用全局旋转、缩放和平移的变换。

#     此类用于数据增强，通过在三维空间中对点云及其对应的边界框进行旋转、缩放和平移操作，提高模型对这些变化的鲁棒性。
#     """
...
#                  translation_std: List[int] = [0, 0, 0],
#                  shift_height: bool = False) -> None:
#         # 初始化函数，设置旋转范围、缩放范围、平移标准差和是否改变高度
#         seq_types = (list, tuple, np.ndarray)  # 序列类型，用于类型检查
#         # 确保旋转范围是序列类型，否则抛出类型错误
#         if not isinstance(rot_range, seq_types):
#             assert isinstance(rot_range, (int, float)), \
#                 f'unsupported rot_range type {type(rot_range)}'
#             rot_range = [-rot_range, rot_range]
...
#             'translation_std should be positive'
#         self.translation_std = translation_std
#         self.shift_height = shift_height  # 是否在缩放时改变高度

#     def _trans_bbox_points(self, input_dict: dict) -> None:
#         """对边界框和点云进行平移处理。
#         从高斯分布中抽样平移向量，并应用于点云和边界框。
#         """
#         translation_std = np.array(self.translation_std, dtype=np.float32)
#         trans_factor = np.random.normal(scale=translation_std, size=3).T
#         input_dict['points'].translate(trans_factor)  # 对点云进行平移
#         input_dict['pcd_trans'] = trans_factor  # 记录平移向量
#         if 'gt_bboxes_3d' in input_dict:  # 如果有边界框数据，也进行平移
#             input_dict['gt_bboxes_3d'].translate(trans_factor)
...
#             points, rot_mat_T = input_dict['gt_bboxes_3d'].rotate(
#                 noise_rotation, input_dict['points'])
#             input_dict['points'] = points  # 更新点云数据
#         else:
#             rot_mat_T = input_dict['points'].rotate(noise_rotation)
#         input_dict['pcd_rotation'] = rot_mat_T  # 记录旋转矩阵
#         input_dict['pcd_rotation_angle'] = noise_rotation  # 记录旋转角度

#     def _scale_bbox_points(self, input_dict: dict) -> None:
#         """对边界框和点云进行缩放处理。
#         根据设定的缩放因子随机选择一个值进行缩放。
#         """
#         scale = input_dict['pcd_scale_factor']
#         points = input_dict['points']
#         points.scale(scale)  # 对点云进行缩放
#         if self.shift_height:
#             assert 'height' in points.attribute_dims.keys(), \
#                 '设置 shift_height=True 但是 points 中没有高度属性'
#             points.tensor[:, points.attribute_dims['height']] *= scale
#         input_dict['points'] = points
#         if 'gt_bboxes_3d' in input_dict and len(input_dict['gt_bboxes_3d'].tensor) != 0:
#             input_dict['gt_bboxes_3d'].scale(scale)  # 对边界框进行缩放
...
#         scale_factor = np.random.uniform(self.scale_ratio_range[0],
#                                          self.scale_ratio_range[1])
#         input_dict['pcd_scale_factor'] = scale_factor  # 记录缩放因子

#     def transform(self, input_dict: dict) -> dict:
#         """对边界框和点云进行旋转、缩放和平移的综合处理。
#         """
#         if 'transformation_3d_flow' not in input_dict:
#             input_dict['transformation_3d_flow'] = []

#         self._rot_bbox_points(input_dict)  # 执行旋转
#         if 'pcd_scale_factor' not in input_dict:
#             self._random_scale(input_dict)  # 如果未设置缩放因子，随机生成
#         self._scale_bbox_points(input_dict)  # 执行缩放
#         self._trans_bbox_points(input_dict)  # 执行平移

#         input_dict['transformation_3d_flow'].extend(['R', 'S', 'T'])  # 记录变换流程
#         return input_dict
...
class ResizeV2:
    ...
    def _resize_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            print("\n======img list len {} =======".format(len(img_list)))
            print("\n======img0 shape {} =======".format(img_list[0].shape))
            new_img_list = []
            for img in img_list:
                if self.keep_ratio:
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: Resized results, 'img_shape', 'pad_shape', 'scale_factor', \
                'keep_ratio' keys are added into result dict.
        """
        print("======== ResizeV2 results KEYS {} ========".format(results.keys()))
...
# 重写一个图像resize 用于nus数据集多视图裁减
@TRANSFORMS.register_module()
class ResizeNus(BaseTransform):
    ...
    def _resize_img(self, results: dict) -> None:
        """Resize images with ``results['scale']``."""
        # nuScenes img 是一个list,第一维度为6,然后是(900,1600,3)
        # 这里需要改写成多图像的处理方式
    ...
    def _resize_bboxes(self, results: dict) -> None:
        ...
        if results.get('gt_bboxes', None) is not None:
            # NOTE hard code for the last dimension
            # nus gt bboxes shape is (n,5)
            # 只取前四维度
            if results['gt_bboxes'].shape[-1] == 5:
                results['gt_bboxes'] = results['gt_bboxes'][:, :4] 
        ...
        if results['flip']:
            ...
            for key in results.get('img_fields', ['img']):
                if type(results[key]) == list:
                    results[key] = [mmcv.imflip(each_img, direction=results['flip_direction'])\
                         for each_img in results[key]]
                else:
...
@TRANSFORMS.register_module()
class NusRandomFlip3D(RandomFlip):
    """Flip the points & bbox.
    ...
    def __init__(self,
        ...
        # for flip prob of 2d image when
        # `sync_2d` is True
        super(NusRandomFlip3D, self).__init__(
            prob=flip_ratio_bev_horizontal, direction='horizontal', **kwargs)
        self.sync_2d = sync_2d
        self.flip_ratio_bev_horizontal = flip_ratio_bev_horizontal
        self.flip_ratio_bev_vertical = flip_ratio_bev_vertical
