...
from mmdet3d.evaluation import kitti_eval,kitti_eval_rot
...
# 读如input dict里的旋转Box而不是从anno里加载Box
@METRICS.register_module()
class KittiMetricRot(BaseMetric):
    ...
    def __init__(self,
        ...
        super(KittiMetricRot, self).__init__(
    ...
    def process_ori(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        """
        print("=============================kitti_metric_rot.py Process================================")
        print("==================== len of data_samples : {} ======================".format(len(data_samples)))
        print(" data_samples[0].keys() : {} ".format(data_samples[0].keys()))
        # printDict(data_samples[0])
        # transform流程
        print("transformation_3d_flow : {} ".format(data_samples[0]["transformation_3d_flow"]))
        # 看看旋转矩阵
        print("pcd_rotation : {} ".format(data_samples[0]["pcd_rotation"]))
        # 旋转角度
        print("pcd_rotation_angle : {} ".format(data_samples[0]["pcd_rotation_angle"]))
        # pcd trans        
        print("pcd_trans : {} ".format(data_samples[0]["pcd_trans"]))
        # pcd scale factor pcd_scale_factor 
        print("pcd_scale_factor : {} ".format(data_samples[0]["pcd_scale_factor"]))
        # gt 3d 是空的
        # gt_instances_3d = data_samples[0]["gt_instances_3d"]
        # print(" gt_instances_3d : {} ".format(gt_instances_3d))
        # gt 2d 是空的
        # gt_instances = data_samples[0]["gt_instances"]
        # print(" gt_instances : {} ".format(gt_instances))
        # pred_3d = data_samples[0]["pred_instances_3d"]
        # print(" pred_3d : {} ".format(pred_3d))
        # datasamples是模型的预测结果, 经过旋转的
        # 由于batch是1, 因此实际上就只有一个样本
        for data_sample in data_samples:
            result = dict()
            pred_3d = data_sample['pred_instances_3d']
            pred_2d = data_sample['pred_instances']
            print("================")
            print("pred_3d : {}--{} ".format(type(pred_3d), pred_3d))
            print("pred_2d : {}--{} ".format(type(pred_2d), pred_2d))
            print("================")
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            pred_3d = data_sample['pred_instances_3d']
            pred_2d = data_sample['pred_instances']
            # 取得旋转角度,平移factor,trans factor
            pcd_rotation_angle = data_sample["pcd_rotation_angle"]
            pcd_trans = data_sample['pcd_trans']
            pcd_scale_factor = data_sample['pcd_scale_factor']
            # 对预测框逆向旋转,平移
            inverse_angle = -pcd_rotation_angle
            inverse_trans = -pcd_trans
            inverse_scale = 1/pcd_scale_factor
            # print("inverse_trans : {}".format(inverse_trans))
            # print("inverse_scale : {}".format(inverse_scale))
            # print("pcd_rotation_angle : {} ; inverse_angle : {}".format(pcd_rotation_angle, inverse_angle))
            # print("Before rotation:", pred_3d)
            # 转流程: T ---> S ---> R
            bboxes_3d = pred_3d["bboxes_3d"]
            # 先放到cpu上
            bboxes_3d_cpu = bboxes_3d.cpu()
            # print("bboxes_3d_cpu : ",bboxes_3d_cpu)
            # 平移回去
            # bboxes_3d_cpu = LiDARInstance3DBoxes()
            bboxes_3d_cpu.translate(inverse_trans)
            # 缩放回去
            bboxes_3d_cpu.scale(inverse_scale)
            # 旋转回去
            bboxes_3d_cpu.rotate(inverse_angle)

            pred_3d["bboxes_3d"] = bboxes_3d_cpu.cuda()

            # print("After rotation:", pred_3d)
    ...
    def compute_metrics(self, results: List[dict]) -> Dict[str, float]:
        ...
        """
        # 每帧处理的时候没进,应该是最后统一计算
        # print("=============================kitti_metric_rot.py compute_metrics================================")
        ...
        # =============================gt_annos list len : 3769================================
        # =============================gt_annos0 type : <class 'dict'>================================
        # =============================gt_annos0 keys : dict_keys(['name', 'truncated', 'occluded', 'alpha', 'bbox', 'location', 'dimensions', 'rotation_y', 'score'])================================        
        # print("=============================gt_annos list len : {}================================".format(len(gt_annos)))
        # print("=============================gt_annos0 type : {}================================".format(type(gt_annos[0])))
        # print("=============================gt_annos0 keys : {}================================".format(gt_annos[0].keys()))
        # # 这一部还没有变成边界框
        # print("=============================result_dict keys : {}================================".format(result_dict.keys()))
    ...
    def kitti_evaluate(self,
        ...
        for name in results_dict:
            if name == 'pred_instances' or metric == 'img_bbox':
                ...
                # 修改为rot
            ap_result_str, ap_dict_ = kitti_eval_rot(
