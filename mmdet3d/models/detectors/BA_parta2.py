...
@MODELS.register_module("BAPartA2")
class BAPartA2(TwoStage3DDetector):
    r"""Bi-Attention Fusion based on Part-A2 detector.
    ...
    def __init__(self,
        ...
        print("=======================BAPartA2 for detectors.BA_parta2 is used====================")
        pass
    ...
    def extract_feat(self, batch_inputs_dict: Dict) -> Dict:
        ...
        # 取出neck feature
        x = neck_feats[0]
        # print("=========parta2 neck_feats shape : {}========".format(x.shape))

        # 如果有图像分支就进入图像分支
        if self.with_img_backbone:
            # 从inputs中获取输入图像
            imgs = batch_inputs_dict.get('imgs', None)
            # print("==================BAPartA2 imgs : {}==================".format(imgs.shape))
            # voxel特征的shape
            height = x.shape[2]
            width = x.shape[3]
            channels = x.shape[1]
...
@MODELS.register_module("BAPartA2S")
class BAPartA2S(TwoStage3DDetector):
    r"""Bi-Attention Fusion based on Part-A2 detector.
    ...
    def __init__(self,
        ...
        print("=======================BAPartA2S for detectors.BA_parta2 is used====================")
        pass
        ...
        if self.with_img_backbone:
            ...
            if self.with_cross_attention:
                ...
                if self.attention_type == "SoftmaxCrossAttention" or "CrossFocusedLinearAttention":
                    ...
                    # 点云增强的图像特征同点云进一步融合
                    # key作query,因为没有添加位置编码
                    fusion_feature = self.cross_attention(key_pts_feat,key_img_feats,value_img_feats,H=H,W=W)
                    pass
                else:
                    # fusion_feature = self.cross_attention(value_img_feats,key_img_feats,encoded_pts_feat)
                    raise ValueError("self.attention_type must be SoftmaxCrossAttention or CrossFocusedLinearAttention!")
