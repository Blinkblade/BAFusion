...
def get_conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias,
    ...
    """
    根据输入的参数创建一个2D卷积层，根据kernel_size和padding等参数尝试使用优化后的iGEMM实现。
    ...
    Args:
        ...
        padding (Union[int, Tuple[int, int], None]): 填充的大小，默认为None时表示自动填充到使卷积输出与输入具有相同宽高。
        dilation (Union[int, Tuple[int, int]]): 卷积核的膨胀大小。
        groups (int): 控制输入和/或输出之间的连接。默认为1，表示全连接。
        bias (bool): 是否添加偏置项。
        attempt_use_lk_impl (bool, optional): 是否尝试使用iGEMM实现优化大核卷积。默认为True。
    
    Returns:
        Union[nn.Conv2d, DepthWiseConv2dImplicitGEMM]: 返回创建的2D卷积层，如果满足条件，则使用DepthWiseConv2dImplicitGEMM实现。
...
def fuse_bn(conv, bn):
    """
    将卷积层（conv）和批量归一化层（bn）合并为一个新的卷积层。
    
    Args:
        conv (nn.Conv2d): 待合并的卷积层。
        bn (nn.BatchNorm2d): 与卷积层相连接的批量归一化层。
    
    Returns:
        tuple: 包含两个元素的元组，第一个元素是合并后的卷积层的权重，
              第二个元素是合并后的卷积层的偏置项。
    
        - weight (Tensor): 合并后的卷积层的权重，形状与原始卷积层的权重相同。
        - bias (Tensor): 合并后的卷积层的偏置项，形状与原始卷积层的偏置项（如果存在）相同。
...
# 膨胀卷积即空洞卷积,每个元素之间设置一些间隔
def convert_dilated_to_nondilated(kernel, dilate_rate):
    """
    将膨胀卷积核转换为非膨胀卷积核。
    
    Args:
        kernel (torch.Tensor): 膨胀卷积核，形状为 [C_in, C_out/groups, H, W]，其中C_in为输入通道数，C_out为输出通道数，groups为分组数，
            H和W分别为卷积核的高度和宽度。
        dilate_rate (int): 膨胀率，即卷积核中元素之间的间隔。
    
    Returns:
        torch.Tensor: 转换后的非膨胀卷积核，形状与输入kernel相同。
    
    """
    # 创建一个形状为 [1, 1, 1, 1] 的全1张量，并将其移动到与kernel相同的设备上
    identity_kernel = torch.ones((1, 1, 1, 1)).to(kernel.device)
    # 如果kernel的第二维（即通道数）为1，表示这是一个Depthwise Convolution的卷积核
    if kernel.size(1) == 1:
        #   这是一个DW卷积核
        # 使用转置卷积（反卷积）操作将膨胀卷积核转换为非膨胀卷积核
        #   This is a DW kernel
        dilated = F.conv_transpose2d(kernel, identity_kernel, stride=dilate_rate)
        return dilated
    else:
        # 这是一个Dense或者Group-wise（但不是DW）的卷积核
        # 初始化一个空列表，用于存储每个通道的膨胀卷积结果
        slices = []
        # 遍历kernel的每个通道
        for i in range(kernel.size(1)):
            # 使用转置卷积（反卷积）操作将当前通道的膨胀卷积核转换为非膨胀卷积核
            #   This is a dense or group-wise (but not DW) kernel
            dilated = F.conv_transpose2d(kernel[:,i:i+1,:,:], identity_kernel, stride=dilate_rate)
            # 将当前通道的膨胀卷积结果添加到列表中
            slices.append(dilated)
        # 将列表中所有通道的膨胀卷积结果拼接起来，得到最终的非膨胀卷积核
...
def merge_dilated_into_large_kernel(large_kernel, dilated_kernel, dilated_r):
    ...
    Args:
        large_kernel (torch.Tensor): 大的卷积核，其形状应为[batch_size, in_channels, kernel_size, in_channels]。
        dilated_kernel (torch.Tensor): 膨胀卷积核，其形状应为[batch_size, in_channels, dilated_kernel_size, in_channels]。
        dilated_r (int): 膨胀率，即膨胀卷积核的间隔。
    
    Returns:
        torch.Tensor: 合并后的卷积核，其形状与large_kernel相同。
...
class DilatedReparamBlock(nn.Module):
    ...
    def forward(self, x):
        # print("========Dilated Reparam Block x : {}==========".format(x.shape))
        ...
        for k, r in zip(self.kernel_sizes, self.dilates):
            ...
            out = out + bn(conv(x))
        # print("========Dilated Reparam Block output : {}==========".format(out.shape))
        return out
...
class UniRepLKNetBlock(nn.Module):
    ...
    def __init__(self,
        ...
        Args:
            ...
            drop_path (float, optional): DropPath 的丢弃率。默认为 0.，表示不使用 DropPath。
            layer_scale_init_value (float, optional): LayerScale 的初始化值。默认为 1e-6。
            deploy (bool, optional): 是否为部署模式。默认为 False。
            attempt_use_lk_impl (bool, optional): 是否尝试使用LK卷积实现。默认为 True。
            with_cp (bool, optional): 是否使用checkpoint机制来减少显存消耗。默认为 False。
            use_sync_bn (bool, optional): 是否使用同步的 Batch Normalization。默认为 False。
            ffn_factor (int, optional): FFN层的缩放因子。默认为 4。
    ...
    def compute_residual(self, x):
        ...
        Returns:
            Tensor: 残差块的输出张量，形状与输入x相同。
        
        """
        # 通过self.dwconv对输入x进行卷积操作，然后通过self.norm进行归一化
        y = self.se(self.norm(self.dwconv(x)))
        # print("======computing residual y1 : {}======".format(y.shape))
        # 对y进行两次点卷积操作，并在第二次点卷积后进行激活函数处理
        y = self.pwconv2(self.act(self.pwconv1(y)))
        # print("======computing residual y2 : {}======".format(y.shape))
        # 如果self.gamma不为None
        if self.gamma is not None:
            # 将self.gamma重新调整形状，并乘以y
            y = self.gamma.view(1, -1, 1, 1) * y
        # print("======computing residual y3 : {}======".format(y.shape))
        # print("======computing residual y4 : {}======".format(self.drop_path(y).shape))
        # 对y进行drop_path操作
    ...
    def forward(self, inputs):
        ...
        Returns:
            torch.Tensor: 输出张量，为输入张量加上计算残差的结果。
        
        """
        # 定义一个内部函数 _f，用于计算输入 x 加上残差
        def _f(x):
            # 返回 x 加上计算残差的结果
            return x + self.compute_residual(x)

        # 判断是否需要使用检查点（checkpoint）功能
        if self.with_cp and inputs.requires_grad:
            # 如果需要，则使用 checkpoint 包裹 _f 函数，并传入 inputs 作为参数
            out = checkpoint.checkpoint(_f, inputs)
        else:
            # 否则，直接调用 _f 函数并传入 inputs
            out = _f(inputs)
        # print("======== UniRepLKNetBlock out {}=========".format(out.shape))
        # 返回结果
        return out
...
#         Args:
#             in_chans (int, optional): 输入通道数。默认为3。
#             num_classes (int, optional): 分类类别数。默认为1000。
#             depths (tuple[int], optional): 每个阶段的深度（块的数量）。默认为(3, 3, 27, 3)。
#             dims (tuple[int], optional): 每个阶段的通道数。默认为(96, 192, 384, 768)。
#             drop_path_rate (float, optional): DropPath的丢弃率。默认为0.0。
#             layer_scale_init_value (float, optional): LayerScale的初始值。默认为1e-6。
#             head_init_scale (float, optional): 头部权重的初始缩放值。默认为1.0。
#             kernel_sizes (tuple[tuple[int]], optional): 每个阶段的卷积核大小。默认为None。
#             deploy (bool, optional): 是否用于部署模式。默认为False。
#             with_cp (bool, optional): 是否使用Checkpoint。默认为False。
#             init_cfg (dict | None, optional): 初始化配置。默认为None。
#             attempt_use_lk_impl (bool, optional): 是否尝试使用LK实现的Block。默认为True。
#             use_sync_bn (bool, optional): 是否使用SyncBatchNorm。默认为False。
#             **kwargs: 其他关键字参数。
...
#         Raises:
#             ValueError: 如果给定的深度没有默认的卷积核大小设置，并且没有指定每个块的卷积核大小。
...
#         # 下采样模块, 采用layer norm,
#         # kernel=3 stride=2 padding=1 保持特征图不变
...
#         cur = 0
#         # 堆叠UniRepLKNetBlock模块,包含四个stage, 
#         # 每个stage包含depths[i]个UniRepLKNetBlock模块
#         # 看了一下默认设置,一般堆叠2~3个即可
#         for i in range(4):
...
# UniRepLKNet层,用于特征融合卷积
@MODELS.register_module(name="UniRepLKLayers")
class UniRepLKLayers(nn.Module):
    r""" UniRepLKNet
        A PyTorch impl of UniRepLKNet
    ...
    def __init__(self,
        ...
        Args:
            ...
            layer_scale_init_value (float, optional): LayerScale的初始值。默认为1e-6。
            head_init_scale (float, optional): 头部权重的初始缩放值。默认为1.0。
            kernel_sizes (tuple[tuple[int]], optional): 每个阶段的卷积核大小。默认为None。≥7比较合适
            deploy (bool, optional): 是否用于部署模式。默认为False。
            with_cp (bool, optional): 是否使用Checkpoint。默认为False。
            init_cfg (dict | None, optional): 初始化配置。默认为None。
            attempt_use_lk_impl (bool, optional): 是否尝试使用LK实现的Block。默认为True。
        ...
        if kernel_sizes is None:
            if depths in default_depths_to_kernel_sizes:
                ...
                raise ValueError('no default kernel size settings for the given depths, '
                                 'please specify kernel sizes for each block, e.g., '
                                 '((3, 3), (13, 13), (13, 13, 13, 13, 13, 13), (13, 13))')
        print('=========== URLKLayers Kernel_size: ', kernel_sizes)
        for i in range(1):
            assert len(kernel_sizes[i]) == depths[i], 'kernel sizes do not match the depths'
        ...
        # 堆叠UniRepLKNetBlock模块,包含四个stage, 
        # 每个stage包含depths[i]个UniRepLKNetBlock模块
        # 看了一下默认设置,一般堆叠2~3个即可, 这个通过设置depths得到
        # 实际上我们的depths只有一个而不是原版的四个因此只有一次循环
        for i in range(1):
            main_stage = nn.Sequential(
                *[UniRepLKNetBlock(dim=dims[i], kernel_size=kernel_sizes[i][j], drop_path=dp_rates[cur + j],
                                   layer_scale_init_value=layer_scale_init_value, deploy=deploy,
                                   attempt_use_lk_impl=attempt_use_lk_impl,
                                   with_cp=with_cp, use_sync_bn=use_sync_bn, use_dw=use_dw) 
                                   for j in range(depths[i] - 1) ])
            # 堆叠了main stage UniRepLKNetBlock模块后, 还需要堆叠一个压缩层将通道压缩到一半
            j = depths[i] - 1
            # print("========= compress conv params : in_channels = {} , \
            #       out_channels = {} , kernel_size = {}".format(dims[i], out_chans, kernel_sizes[i][j]))
            main_stage.append(
                nn.Conv2d(in_channels=dims[i], out_channels=out_chans, 
                          kernel_size=kernel_sizes[i][j], stride=1, padding=0)
            )
        ...
        # 用默认的初始化,不用mm的初始化
        self.init_cfg = None
        # 由于最终输出并非一维cls，因此不能用这个layernorm
        self.norm = get_bn(dim = last_channels, use_sync_bn=use_sync_bn)
        # self.norm = nn.LayerNorm(last_channels, eps=1e-6)  # final norm layer
        self.apply(self._init_weights)
        # self.output_mode = 'logits'
    ...
    def init_weights(self):
        # mm init
        print("================ There is No MM Init, please cancel relative setting =================")
        pass
    ...
    def forward(self, x):
        # print("============UniRepLKNet x : ", x.shape)
        # 堆叠了4个stage, 每个stage包含depths[i]个UniRepLKNetBlock模块
        ...
        x = self.norm(x)
        # print("============UniRepLKNet output : ", x.shape)
        return x
