...
class Anchor3DHead(Base3DDenseHead, AnchorTrainMixin):
    ...
    Args:
        ...
        num_classes (int)：目标的类别数。
        in_channels (int)：输入特征图的通道数。
        feat_channels (int)：特征图的通道数。
        use_direction_classifier (bool)：是否添加方向分类器。
        anchor_generator (dict)：锚点生成器的配置字典，负责生成用于检测的3D锚点。
        assigner_per_size (bool)：是否为每个不同的锚点大小进行单独的分配。
        assign_per_class (bool)：是否为每个类别进行单独的分配。
        diff_rad_by_sin (bool)：是否将框回归损失中的角度差异转换为正弦差异，这在处理周期性的角度时有帮助，可以使损失函数对于角度的周期性更加鲁棒。
        dir_offset (float | int)：BEV（鸟瞰图）旋转角度的偏移。
        dir_limit_offset (float | int)：BEV旋转角度的限制范围。
        bbox_coder (dict)：框编码器的配置字典，负责将网络预测转换为实际的3D边界框。 DeltaXYZWLHRBBoxCoder
        loss_cls (dict)：分类损失的配置。
        loss_bbox (dict)：定位损失的配置。
        loss_dir (dict)：方向分类器损失的配置。
        train_cfg (dict)：训练配置。
        test_cfg (dict)：测试配置。
    ...
    # 返回 预测结果 cls_score bbox_pred dir_cls_pred(朝向分类)
