...
def clear_instance_unused_keys(instance):
    ...
    for k in keys:
        if instance[k] is None:
            # 即使2d bbox和labels为none仍然保留
            if (k == 'gt_bboxes') or (k == 'gt_labels'):
                print("there is no {} , set to None!".format(k))
                pass
            else:
                del instance[k]
    return instance
...
def update_pkl_infos_nus2d(dataset, out_dir, pkl_path):
    if dataset.lower() == 'kitti':
        ...
        update_nuscenes_infos2D(pkl_path=pkl_path, out_dir=out_dir)
    elif dataset.lower() == 'nuscenes_mini':
        update_nuscenes_infos2D(pkl_path=pkl_path, out_dir=out_dir, dataroot='./data/nuscenes_mini')
